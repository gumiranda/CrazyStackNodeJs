...
describe("addSilvioSantos", () => {
    ...
    beforeAll(async () => {
        ...
        addSilvioSantosRepository.addSilvioSantos.mockResolvedValue(fakeSilvioSantosEntity);
    });
    beforeEach(() => {
    ...
    it("should rethrow if addSilvioSantos of AddSilvioSantosRepository throws", async () => {
        addSilvioSantosRepository.addSilvioSantos.mockRejectedValueOnce(new Error("any_error"));
        await expect(testInstance(fakeSilvioSantosEntity)).rejects.toThrowError("any_error");
    });
});
