...
describe("LoadRequestByPageController", () => {
  ...
  let loadRequestByPage: jest.Mock;
  ...
  beforeAll(async () => {
    ...
    loadRequestByPage = jest.fn();
    loadRequestByPage.mockResolvedValue(fakeRequestPaginated);
    validation = mock();
  ...
  test("should call loadRequestByPage with correct params", async () => {
    ...
    expect(loadRequestByPage).toHaveBeenCalledTimes(1);
  });
  test("should call loadRequestByPage with correct params in desc order", async () => {
    const result = await testInstance.execute({
  ...
  test("should call loadRequestByPage with correct params without http query", async () => {
    const result = await testInstance.execute({
      userId: fakeUserEntity?._id,
    });
    expect(result).toEqual(ok(fakeRequestPaginated));
  });
  test("should throws if loadRequestByPage throw", async () => {
    loadRequestByPage.mockRejectedValueOnce(new Error("error"));
