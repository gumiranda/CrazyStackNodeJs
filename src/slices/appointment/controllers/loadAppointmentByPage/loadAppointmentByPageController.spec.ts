...
import { LoadAppointmentByPageController } from "./loadAppointmentByPageController";
import {
...
describe("LoadAppointmentByPageController", () => {
  let testInstance: LoadAppointmentByPageController;
  let loadAppointmentByPage: jest.Mock;
  ...
  beforeAll(async () => {
    ...
    loadAppointmentByPage = jest.fn();
    loadAppointmentByPage.mockResolvedValue(fakeAppointmentPaginated);
    validation = mock();
  ...
  test("should call loadAppointmentByPage with correct params", async () => {
    ...
    expect(loadAppointmentByPage).toHaveBeenCalledTimes(1);
  });
  test("should call loadAppointmentByPage with correct params in desc order", async () => {
    const result = await testInstance.execute({
  ...
  test("should call loadAppointmentByPage with correct params without http query", async () => {
    const result = await testInstance.execute({
      userId: fakeUserEntity?._id,
    });
    expect(result).toEqual(ok(fakeAppointmentPaginated));
  });
  test("should throws if loadAppointmentByPage throw", async () => {
    loadAppointmentByPage.mockRejectedValueOnce(new Error("error"));
