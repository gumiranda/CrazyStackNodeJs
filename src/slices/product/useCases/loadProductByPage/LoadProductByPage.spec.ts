...
describe("LoadProductByPage", () => {
  ...
  beforeAll(async () => {
    ...
    loadProductByPageRepository.loadProductByPage.mockResolvedValue(fakeProductPaginated);
  });
  beforeEach(() => {
  ...
  it("should call loadProductByPage of LoadProductByPageRepository with correct values", async () => {
    ...
    expect(loadProductByPageRepository.loadProductByPage).toHaveBeenCalledWith(fakeQuery);
