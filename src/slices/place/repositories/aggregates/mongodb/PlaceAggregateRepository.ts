...
import type { LoadPlaceByPageGeoNearRepository } from "../../contracts";
export class PlaceAggregateRepository implements LoadPlaceByPageGeoNearRepository {
  ...
  async loadPlaceByPageGeoNear(query: Query): Promise<PlacePaginated | null> {
    ...
    if (queryMongo?.$text) {
      ...
      return { places: resultPaginatedArray, total: totalPaginated };
    }
    ...
    return { places: resultGeoNearPaginatedArray, total };
  }
}
