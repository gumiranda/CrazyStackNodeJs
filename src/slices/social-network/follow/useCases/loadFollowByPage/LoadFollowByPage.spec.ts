...
import { fakeFollowPaginated } from "@/slices/follow/entities/FollowEntity.spec";
...
describe("LoadFollowByPage", () => {
    ...
    beforeAll(async () => {
        ...
        loadFollowByPageRepository.loadFollowByPage.mockResolvedValue(
            fakeFollowPaginated
        );
    });
    beforeEach(() => {
    ...
    it("should call loadFollowByPage of LoadFollowByPageRepository with correct values", async () => {
        ...
        expect(loadFollowByPageRepository.loadFollowByPage).toHaveBeenCalledWith(
            fakeQuery
        );
