...
import { UpdateFollow } from "@/slices/social-network/follow/useCases";

export class UpdateFollowController extends Controller {
  constructor(
    private readonly validationQuery: Validation,
    private readonly validationBody: Validation,
    private readonly updateFollow: UpdateFollow
  ) {
    super();
  }
  async execute(httpRequest: HttpRequest<any>): Promise<HttpResponse<any>> {
    ...
    const followUpdated = await this.updateFollow(
      {
        fields: {
          ...httpRequest?.query,
