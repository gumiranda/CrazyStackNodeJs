...
import { fakeTweetPaginated } from "@/slices/tweet/entities/TweetEntity.spec";
...
describe("LoadTweetByPage", () => {
    ...
    beforeAll(async () => {
        ...
        loadTweetByPageRepository.loadTweetByPage.mockResolvedValue(
            fakeTweetPaginated
        );
    });
    beforeEach(() => {
    ...
    it("should call loadTweetByPage of LoadTweetByPageRepository with correct values", async () => {
        ...
        expect(loadTweetByPageRepository.loadTweetByPage).toHaveBeenCalledWith(
            fakeQuery
        );
