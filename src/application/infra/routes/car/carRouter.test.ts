...
describe("Route api/car", () => {
  ...
  beforeEach(async () => {
    ...
    carCollection = await MongoHelper.getCollection("car");
  ...
  describe("POST /api/car/add", () => {
    ...
    test("Should return 400 for bad requests", async () => {
      const { token } = await makeAccessToken("admin", "password");
      const carWrongBody = { ...carBody, name: null };
      const responseAdd = await fastify.inject({
        method: "POST",
        url: "/api/car/add",
        headers: { authorization: `Bearer ${token}` },
        payload: carWrongBody,
      });
  ...
  describe("GET /api/car/load", () => {
    ...
    test("Should return 200 on load", async () => {
      const { insertedId } = await carCollection.insertOne(carBody);
      const { token } = await makeAccessToken("admin", "password");
      const response = await fastify.inject({
        method: "GET",
        url: `/api/car/load?_id=${insertedId.toString()}`,
        headers: { authorization: `Bearer ${token}` },
      });
    ...
    test("Should return 401 for unauthorized access token", async () => {
      const response = await fastify.inject({
        method: "GET",
        url: `/api/car/load?_id=${new ObjectId().toString()}`,
        headers: { authorization: "Bearer invalid_token" },
      });
      expect(response.statusCode).toBe(401);
    });
  ...
  describe("GET /api/car/loadByPage", () => {
    test("Should return 400 for bad requests", async () => {
      const { token } = await makeAccessToken("admin", "password");
      const response = await fastify.inject({
        method: "GET",
        url: "/api/car/loadByPage",
        headers: { authorization: `Bearer ${token}` },
      });
      expect(response.statusCode).toBe(400);
    });
    test("Should return 200 on loadByPage", async () => {
      await carCollection.insertOne(carBody);
      const { token } = await makeAccessToken("admin", "password");
      const response = await fastify.inject({
        method: "GET",
        url: `/api/car/loadByPage?page=${1}`,
        headers: { authorization: `Bearer ${token}` },
      });
      const responseBody = JSON.parse(response.body);
      expect(response.statusCode).toBe(200);
      expect(responseBody.cars).toBeTruthy();
  ...
  describe("DELETE /api/car/delete", () => {
    test("Should return 400 for bad requests", async () => {
      const { token } = await makeAccessToken("admin", "password");
      const response = await fastify.inject({
        method: "DELETE",
    ...
    test("Should return 200 on delete", async () => {
      ...
      const { insertedId } = await carCollection.insertOne({
        ...carBody,
        createdById: _id,
      });
      const response = await fastify.inject({
        method: "DELETE",
        url: `/api/car/delete?_id=${insertedId.toString()}`,
        headers: { authorization: `Bearer ${token}` },
      });
      const responseBody = JSON.parse(response.body);
      expect(response.statusCode).toBe(200);
    ...
    test("Should return 401 for unauthorized access token", async () => {
      const response = await fastify.inject({
        method: "DELETE",
        url: `/api/car/delete?_id=${new ObjectId().toString()}`,
        headers: { authorization: "Bearer invalid_token" },
      });
      expect(response.statusCode).toBe(401);
    });
  ...
  describe("PATCH /api/car/update", () => {
    ...
    test("Should return 200 on update", async () => {
      ...
      const response = await fastify.inject({
        method: "PATCH",
        url: `/api/car/update?_id=${insertedId.toString()}`,
        headers: { authorization: `Bearer ${token}` },
        body: { name: "new name" },
      });
      const responseBody = JSON.parse(response.body);
    ...
    test("Should return 401 for unauthorized access token", async () => {
      const response = await fastify.inject({
        method: "PATCH",
        url: `/api/car/update?_id=${new ObjectId().toString()}`,
        headers: { authorization: "Bearer invalid_token" },
        body: { name: "new name" },
      });
      expect(response.statusCode).toBe(401);
